...
| **D12** | **Two units, not an end-to-end rebuild.** The walk D11 deleted is already covered live: `kb-testing`'s `stage-arxiv-paper` stands up a real consuming repo (git init, unpack, install the set, stub a Makefile) and `no-inference-kb-driver-arxiv-paper` drives the shipped entry point over it with real ledger subprocesses, the repo's own `kb-refresh`/`kb-verify` and every postcondition; `kb-driver-arxiv-corpus` does the same with inference enabled. The dispatching rows are unit-covered with `_fake_model` injected — `ov.docs`/`p5.review`/`p5.fix` in `test_kb_driver_buildout.py` and `test_kb_driver_call.py`, `discover.build` in `test_kb_claimgraph_cinf.py` and `test_kb_driver_checkpoints.py`. **Rebuilding that walk in-process would be a deterministic fake of something already run for real.** Two holes remain. **(a)** `config.invocation`'s wiring into the `BatonContext` `cli.main` renders, which lost its only test with the `start.proceed` barrier. **(b)** A resume whose head stages are *actually recorded* — `test_kb_driver_resume_state.py` covers resume logic against a fake ledger by its own statement, and the live sweeps run straight through to completion, so no tier exercises it. `ARCHITECTURE.md`'s "the resume entry … has no suite" is current, not stale | PC | — | (a) and (b) covered as unit tests. `ARCHITECTURE.md`'s no-suite sentence disposed of by (b) landing |
...
**Everything except D12 has landed.** D12 is the only row that adds code rather than removing it, and it is two unit tests rather than the integration rebuild it was first written as.
