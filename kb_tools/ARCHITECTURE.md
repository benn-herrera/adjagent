...
**What the sheet elides, the census counts.** The `FACT edges` line reports the
strokes drawn and, beside them, how many premises a drawn route already carried
(`ops._census`) — the count SPEC.md, The Claim-Graph Sheet puts in place of a
picture that cannot show them without drawing the strokes it just left out. It
is the corpus's figure rather than the selected view's, as the
